...
enum MetrologyLedger {
    ...
    @MainActor
    static func calibrate() async -> GaugeVerdict {
        await CaliperSweep().run()
    }
}

enum GaugeVerdict: Equatable {
    case shifted(URL)
    case aligned
    case blank
}
...
@MainActor
final class CaliperSweep: NSObject, WKNavigationDelegate {
    private var continuation: CheckedContinuation<GaugeVerdict, Never>?
    private var probe: WKWebView?
    private var settled = false
    private var watchdog: Task<Void, Never>?

    func run() async -> GaugeVerdict {
    ...
    private func resolve(_ verdict: GaugeVerdict) {
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction,
        ...
        if MetrologyLedger.fingerprint(target) != MetrologyLedger.fingerprint(anchor) {
